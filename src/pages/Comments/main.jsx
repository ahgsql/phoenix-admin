            ...
            if (!commentsFromDb) {
                ...
                for (const comment of commentsFromDb) {
                    try {
                        ...
                        console.log("await of un içeriği", productData);
                        count -= 1
    ...
    return (
        <div id="CommentContainer">
            {comments.map((comment) => {
                return (
                    <div className="CommentRow" id={comment._id}>
                        <h3>{comment.userName}</h3>
                        <span className="time">{timeAgo.format(new Date(comment.createdAt))}</span>
                        {comment.productTitle && <h4>Product: {comment?.productTitle}</h4>}
                        <p>{comment.content}</p>
                        <div className="navigationButtons">
                            <button style={{backgroundColor:"#3498db"}} onClick={() => navigate("/products/detail/"+comment.productId)}>Product Detail</button>
